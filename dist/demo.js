...
// 导入 Ashare SDK 函数
const index_1 = require("./src/index"); // BOLL 等指标从 index 导出
const utils_1 = require("./src/utils"); // MA 等基础工具函数从 utils 导出
// 辅助函数：打印数据摘要
...
function runDemo() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const shIndexDaily = yield (0, index_1.get_price)('sh000001');
        printDataSummary('上证指数日线行情 (默认)', shIndexDaily);
        // 获取上证指数日线行情 (指定 count=5)
        const shIndexDaily5 = yield (0, index_1.get_price)('sh000001', '', 5);
        printDataSummary('上证指数日线行情 (count=5)', shIndexDaily5);
        // 获取上证指数历史日线行情 (指定 end_date 和 count)
        const shIndexHist = yield (0, index_1.get_price)('000001.XSHG', '2023-12-31', 5, '1d');
        printDataSummary('上证指数历史日线行情 (end_date="2023-12-31", count=5)', shIndexHist);
        // 获取贵州茅台15分钟线行情 (count=5)
        const kweiChow15m = yield (0, index_1.get_price)('sh600519', '', 5, '15m');
        printDataSummary('贵州茅台15分钟线 (count=5)', kweiChow15m);
        // 获取贵州茅台60分钟线行情 (count=6)
        const kweiChow60m = yield (0, index_1.get_price)('600519.XSHG', '', 6, '60m');
        printDataSummary('贵州茅台60分钟线 (count=6)', kweiChow60m);
        // --- 复现 Demo2.py (数据处理部分) ---
        // 获取上证指数日线行情 (count=120)
        const shIndexDaily120 = yield (0, index_1.get_price)('000001.XSHG', '', 120, '1d');
        ...
        if (shIndexDaily120.length > 0) {
            ...
            // 计算 BOLL 指标
            const [bollUp, bollMid, bollLower] = (0, index_1.BOLL)(closePrices); // 默认 N=20, P=2
