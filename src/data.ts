 ...
 * @returns 股票数据序列
 */
export async function get_price_day_tx(code: string, end_date: string = '', count: number = 10, frequency: '1d' | '1w' | '1M' = '1d'): Promise<StockData[]> {
  ...
  try {
    ...
    const stockData: StockData[] = buf.map((item: any) => ({
 ...
 * @returns 股票数据序列
 */
export async function get_price_min_tx(code: string, end_date: string = '', count: number = 10, frequency: '1m' | '5m' | '15m' | '30m' | '60m' = '1m'): Promise<StockData[]> {
 ...
 * @returns 股票数据序列
 */
export async function get_price_sina(code: string, end_date: string = '', count: number = 10, frequency: '5m' | '15m' | '30m' | '60m' | '1d' | '1w' | '1M' = '60m'): Promise<StockData[]> {
    ...
    try {
        ...
        const stockData: StockData[] = dstr.map((item: any) => ({
